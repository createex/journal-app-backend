const { goalsCluster } = require("../models");
...
const getAll = async (req, res) => {
  try {
    let data = await goalsCluster.find();
    if (data.length === 0) {
      return res.send({
        message: "Not Found",
        result: false,
...
const getById = async (req, res) => {
  try {
    ...
    let data = await goalsCluster.findOne({ _id: id });
    if (data == null) {
      return res.send({
        message: "Not Found",
        result: false,
...
const deleteById = async (req, res) => {
  try {
    let id = req.params.id;
    let data = await goalsCluster.findByIdAndDelete(id);
    if (data == null) {
      return res.send({
        message: "Not Found",
        result: false,
...
const update = async (req, res) => {
  try {
    ...
    let data = await goalsCluster.findOneAndUpdate(
      { _id: id },
      {
        $set: json,
      },
